...
class Envelope(object):
    """
    Base class that defines the interface for an envelope. 
    An envelope generates some sort of curve over time, like a decay
    curve for instance.
    """
    def __init__(self):
...
class FixedDecay(Envelope):
    """
    An envelope which will decay at a given rate
    """
...
class Instrument(object):
    """
    An Instrument renders notes in some manner. When a note is triggered on
    a track, the Instrument associated with that track will render
    some sort of output, which might be modified by an envelope.

    The basic Instrument class will render the level of the note as
    an interpolation between the background and the foreground color using
    the supplied envelope. With no envelope the fg color is rendered at
    the level of the note for the duration of the note.
    """
...
class ShrinkingSpike(Instrument):
    """
    An Instrument which renders the level as a distance down on the spike
    at the foreground color. This can handle a list of lists as a target.
    """
...
class FallingSpike(ShrinkingSpike):
    """
    A type of Spike Instrument which renders the level as a block of
    color which falls down the spike.
    This can handle a list of lists as a target
    """
...
class HueInstrument(Instrument):
    """
    Like the basic instrument but uses the hue of the note instead of the fgcolor,
    and always uses black (value 0) as the bgcolor
    """
...
class TargetRandomizer(object):
    """
    A utility which will modify the target of an instrument so that it's destination
    is random. It's a little hacky in the way it reaches into the instruments
    internal data, but it works.
    """
...
class RandomTargetAndColorInstrument(object):
    """
    A utility which will modify the target of an instrument so that it's destination
    is random. It's a little hacky in the way it reaches into the instruments
    internal data, but it works.
...
class Phrase(object):
    """
    A phrase is a collection of notes, which can be bound onto a track. It is
    independent of any Instrument which will be used to render the notes. The
    notes have both levels and hues.
    """
...
class Track(object):
    """
    A track binds a Phrase to an Instrument. It is the thing which is then
    updated by the main animation loop.
    """
