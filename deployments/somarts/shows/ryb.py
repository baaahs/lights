...
class RYB(looping_show.LoopingShow):
    ...
    def update_at_progress(self, progress, new_loop, loop_instance):
        ...
        if mode == 0:
            ...
            per_slice = v_range / len(geom.ALL)

            for idx, sl in enumerate(geom.ALL):
                ...
                self.ss.party.set_cell(sl, self.finalFromRGB(rgbTuple))


        elif mode == 1:
            # Each icicle gets a unique color based on it's offset
            for idx,icicle in enumerate(geom.ALL):
                ...
                self.ss.party.set_cell(icicle, self.finalFromRGB(rgbTuple))
